    ...
    if (sv_cheats.GetBool() && (sar_seamshot_finder.GetBool() || sar_seamshot_helper.GetBool())) {
        ...
        if (tr.plane.normal.Length() > 0.9) {
            ...
            if (sar_seamshot_helper.GetBool()) {
                int uiScale = 5;
                engine->AddLineOverlay(tr.endpos, tr.endpos + tr.plane.normal * uiScale, 0, 0, 255, true, 0.06);
                engine->AddLineOverlay(tr.endpos - checkDirs[0] * uiScale, tr.endpos + checkDirs[0] * uiScale, 0, 255, 0, true, 0.06);
                engine->AddLineOverlay(tr.endpos - checkDirs[1] * uiScale, tr.endpos + checkDirs[1] * uiScale, 255, 0, 0, true, 0.06);
            }
        }
    }
}